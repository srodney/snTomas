...
As in \citet{Patel:2014}, we derive a distance modulus 
from the SALT2 fit using\footnote{This avoids confusion by reserving the symbol $\mu$
to refer to the lensing magnification. The 'dm' is a standard distance
modulus, defined as ${\rm dm}=5\log_{10}d_L+25$, where $d_L$ is the
...
in \citet{Patel:2014}.  This correction was derived by applying both
...
Table~\ref{tab:DistanceAndMagnification}, both for the spectroscopic
...
follow \citet{Patel:2014} and define the magnification by comparing
...
Table~\ref{tab:DistanceAndMagnification} under the ``Control'' column.
...
Table~\ref{tab:DistanceAndMagnification}.
...
using the SALT2 fitter (top panel) and the MLCS2k2 fitter (right).
