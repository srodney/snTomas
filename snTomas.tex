...
possible to break this degeneracy \citep{Seitz:1997,Bradac:2004}.  However, there are more complex versions
...
background source's flux and size. \change{In the case of the simple mass sheet degeneracy described above, the magnification scales as $\mu\propto(1-\lambda)^{-2}$ \citep[see e.g.][]{Bartelmann:2010}.} Therefore, an absolute measurement
...
the galaxy cluster Abell 2744.  Sections~\ref{sec:Spectroscopy}
...
tension between our magnification measurement and the lens models,
and our conclusions are summarized in Section~\ref{sec:SummaryAndConclusions}.
with implications for the systematic error budget in magnification
estimates for other high-$z$ lensed sources.
...
$10^{10.1}$ \Msun\ based on Eq. 7 of \citet{Taylor:2011}, using the
...
magnification measurement was known.  The final two models (Bradac-v2
and Lam-v2) are technically not blind, although none of these modelers
used the SN magnification as a constraint, and the modelers did not
have access to the final SN magnification value when constructing
their model.}
...
generally using a multi-scale grid to get better sampling in regions with a higher density of information. Each grid cell is assigned a mass or a potential, and then
...
\change{ In general, free-form methods tend to be more flexible than 
parametric methods, as they are not guided (or restricted) by
assumptions about the mass distribution of dark matter halos. }
...
expected to improve the lens models, and in
Figure~\ref{fig:LensingTest} we see that it does: the mean for the
post-HFF sample is $\mu_{\rm post}=2.3\pm0.6$, fully consistent with
the observed SN magnification.}
...
%also use weak-lensing measurements (diamonds).  
...
\change{ Of the 15 models examined here, both of the Zitrin models 
...
redshift. Here we see evidence for a correlation between model
accuracy and the fraction of multiply-imaged galaxies with
spectroscopic redshifts.  The apparent correlation is anchored on one
end by the low spectroscopic fraction and high tension for the
CATS(v2) model. At the other end we find two of the most accurate lens
models (Bradac(v2) and Lam(v2)), which both have a high spec-$z$
fraction and low tension.}

\change{
Of course this is not direct causation.  Consider the limit of just a
single multiply-imaged galaxy constraint: the spec-$z$ fraction could
be 1.0 but the model would be wildly unconstrained.  Rather, the
spec-$z$ fraction here serves as a stand-in for how stringently the
strong-lensing constraints are defined in these models.  Erroneous
redshifts assigned to multiply-imaged systems lacking a spec-$z$ are especially
problematic for relatively low redshift background systems. At lower
redshifts, the model-derived deflection angle and magnification show a
more rapid variation with the redshift of the background galaxy than
for similarly positioned high redshift background galaxies}
