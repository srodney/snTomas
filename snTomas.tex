...
presented in Figure~\ref{fig:DiscoveryImage}, \changeR{and the
...
with that redshift, meaning that this is a background
...
eight \change{so-called} {\it parametric}\footnote{\change{Although
this nomenclature is becoming standard in the literature, it is
somewhat misleading, as the pixels or grid cells in free-form models
are effectively parameters as well. Perhaps ``simply-parameterized''
would be more accurate, though we adopt the more common usage here.}}
...
\changeR{cluster's dark matter can be described
by analytic forms such as NFW halos \citep{Navarro:1997}, or pseudo isothermal elliptical mass distributions \citep{Kassiola:1993}.}
...
particularly acute for models that \changeR{hew more closely to a
...
primary methodology as the earlier CATS models, \changeR{though with
...
value by \changeR{$4.1\sigma$, and higher than either of the previous
...
\changeR{Furthermore,} the shift to higher magnifications is not unique to the
...
models.  Although \changeR{the single sight-line to SN \tomas\ } can
only provide a very limited test of the model,
the \changeR{$4.1\sigma$} discrepancy is at least a strong suggestion
that \changeR{even with a large number of high-quality} strong-lensing
constraints \changeR{it is still possible to generate} magnification
...
Taking advantage of the availability of \changeR{14} well-constrained lens
...
evaluating \changeR{seven} possible causes: 
...
\noindent The first \changeR{four} of these take on the discrepancy from 
...
2744 \changeR{could} either incorporate the observed magnification
