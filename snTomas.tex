...
provide valuable leverage for breaking degeneracies that positional
constraints cannot disentangle.}  We compare the measured $\mu$
against predicted magnifications from 15 lens models, \change{ of
which 13 were generated before the SN magnification was known,
qualifying as a pure ``blind test.''}  The models are collectively
fairly accurate: \change{6 of the blind models and both of the post-SN
...
for strong-lensing constraints. 
...
and expect to detect the reappearance of this object (called ``SN
...
developed into an important new cosmological tool.
...
mass-sheet degeneracy \citep{Falco:1985,Schneider:1995}.  This
...
constraints from strong and weak lensing \citep{Seitz:1997}.  When lensed background
...
within that redshift range \citep{Bradac:2004}.  However, there are more complex versions
...
position of \tomas.  These models represent a broad sampling of the
...
models, and the remining seven are {\it free-form} models.  Broadly
...
\change{
In general, free-form methods tend to be more flexible than parametric methods,   
as reflected by the
size of the error-bars in Figure~\ref{fig:LensingTest}.
...
returned by all other models.  This is due in part to the 
 inclusion of many systematic error sources. For example, this
...
arbitrarily scaled mass sheet. More complex degeneracies are also accounted for
...
other and with the observed magnification of SN \tomas. \change{If we
take the first set of 9 pre-HFF models and naively treat each as an
independent prediction for the magnification (they are not all
independent) and if we ignore their quoted uncertainties, we would get
an unweighted mean for the pre-HFF sample of $\mu_{\rm
pre}=2.6\pm0.4$.  This is separated from the observed SN magnification
by $\delta\mu/\mu=28\%$, which is approximately a 1.5$\sigma$
difference. The addition of deeper imaging and new spectroscopic
constraints from the HFF program and affiliated efforts should be
expected to improve the lens models, and in fact we see that it does:
the mean for the post-HFF sample is $\mu_{\rm post}=2.3\pm0.6$, fully
consistent with the observed SN magnification.} 

This is largely in agreement with the results
of \citetalias{Patel:2014} and \citet{Nordin:2014}, \change{where
...
with a complex mass distribution, and \change{the SN is located
...
\subsection{Possible Errors in Lens Model Inputs}
\label{sec:LensModelInputs}
...
characteristics of the most accurate models.  In this way SN \tomas\
may serve as a guide toward optimizing future lens models for
...
\change{
\subsubsection{Quantity and Quality of Strong Lensing Constraints}
\label{sec:QuantityAndQuality}
}

\change{
Increasing the number of multiply imaged galaxies used as
strong-lensing constraints should in principle lead to improvements in
the accuracy and precision of magnification predictions.  We have
already seen some suggestion of this from
Figure~\ref{fig:LensingTest}, which shows that the post-HFF models are
collectively better than the pre-HFF models at predicting the
SN \tomas\ magnification.  
The left panel of Figure~\ref{fig:Nspecz} isolates this variable by
plotting the tension between each model prediction and the measured SN
magnification against the number of multiply-imaged galaxies used as
strong-lensing constraints. There is no clear correlation here, and in
fact the most significant outlier (CATS(v2)) is actually the one using
the most multiply-imaged systems.  
}

\change{
In the right panel of
Figure~\ref{fig:Nspecz} we examine an alternative explanation by plotting 
the model tension against the fraction of strong-lensing constraints that have a spectroscopic redshift. 
Here we see a suggestion of a trend, and find that two of the most successful lens
models (Bradac(v2) and Lam(v2)) do share a common characteristic: they
both use a set of multiply-imaged galaxies for which a large fraction
have spectroscopic redshifts. In practice, this means that the
strong-lensing constraints in these models are defined more
stringently. Other models use a larger set of strong-lensing features,
but the dearth of spectroscopic constraints means that it is more
likely that some of those multiply-imaged galaxies are assigned with
incorrect redshifts.
}
...
%plane).  As the overall magnification of a given model
%increases, differences in angle between the delensed
%images of the same source become smaller, corresponding to an overly shallow lensing mass profile. In
%the limit of only a small number of multiple images that cover a
%limited range of source redshift, this results in a tendency to force all
%sources toward the same location in the source plane, thereby
%maximizing the magnification.
%
%Minimizing in the image plane avoids this well known bias but is much
%more difficult (Broadhurst etal 2005 ApJ 621,53, Zitrin etal 2009,
%MNRAS 396,1985) as model generated images must be searched for in the
%image plane near the corresponding observed images.  
%
%Alternatively, one may guard against this bias using a modified source plane minimization that is
%constrained not to generate overly small re-lensed sources  (Sendra etal 2012,MNRAS 437,2642,Diego etal
%2014).
%
%For models that incorporate HFF
%data, this bias should be reduced, as many
%multiply images spread over a wide redshift range will lessen the bias
%towards high magnification. 
%
%
%
