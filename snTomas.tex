...
from \changeR{15 lens models developed by 8 independent groups},
...
the 1$\sigma$ uncertainties.  \changeR{However, the models exhibit a
...
and \change{6 of these models are $\geq1.5\sigma$} above the measured
magnification.  We evaluate possible causes for this mild
bias, \changeR{and reject explanations that invoke modeling errors or
misinterpretation of the SN data. We conclude that -- at least for
this single line of sight -- the most accurate models are those with
(1) stringent quality cuts for strong-lensing constraints and (2)
relatively flexible model design}. \change{A more robust evaluation of
systematic biases could be achieved with further cooperative lens
model comparisons using a larger sample of lensed Type Ia SNe as well
as cluster lensing simulations.}
\end{abstract}
...
\changeG{spectroscopic} redshift constraints, with the most likely host
...
$z=1.5\pm0.2$.  \change{This photo-z was subsequently refined to a
...
The next nearest galaxy detected in HST imaging is \change{2.2\arcsec
...
photometric data from the SN itself are \change{consistent with the
...
that have extended rest-frame UV coverage.  \changeG{The top two panels show
...
match the observed SED of SN \tomas.  \changeG{This approach will find
...
to the unwarped \tomas\ spectrum \changeG{($\chi^2/\nu=220.6/90$,
...
the observed light curve.  \changeG{Taken together with the host galaxy
...
templates to measure the distance modulus \changeG{ and then derive
...
is only 0.06 mag, smaller than the intrinsic dispersion \changeG{of
...
panel).  \changeR{Grey lines with shading show a linear fit to the
...
additional models that were created later.  \changeR{Some of these are
...
magnification and uncertainty for a source at \change{$z=1.3457$} and
...
mass distributions in galaxy clusters. A rigorous comparison of these
disparate lens modeling techniques is beyond the scope of this
work. Table~\ref{tab:PredictedMagnifications} includes a brief
...
Section~\ref{sec:DistanceAndMagnification}.  This comparison shows that these 14
...
of \change{$\mu=2.5\pm0.4$.  This is separated from the observed SN
magnification by $\delta\mu/\mu=23\%$, which is approximately a
1$\sigma$ difference.}
...
more common usage here.}  models on top and seven {\it free-form} models
...
the Williams model (using GRALE), which has the largest uncertainties
is able to account for mass sheet degeneracy by introducing a free
parameter in the reconstruction that represents an arbitrarily scaled
mass sheet, with normalization to be chosen by the genetic algorithm.
More complex degeneracies are also accounted for in this model through
the use of basis
...
\subsection{Possible Errors in Supernova Interpretation}
\label{sec:SupernovaErrors}

\changeR{Before examining the lens models or the cluster, let us first
consider whether we might have misinterpreted the data on the SN
itself.}
...
galaxies.  In this case the redshift \change{is strongly supported} by
...
\subsection{Possible Error in Lens Model Construction}
\label{sec:LensModelErrors}

\changeR{Having now ruled out misinterpetations of the SN
data, we are left to seek an explanation for the small systematic bias
by scrutinizing the lens models.  TODO: note that 
the most successful models are those that use a
stringent criteria for strong lensing constraints, which results in a
larger fraction that have spectroscopic redshifts. 
}
...
acceptable cosmologies \citep[e.g.][]{Hinshaw:2013,Planck-Collaboration:2015},
...
HST-HF-51312 and HST-GO-13386 from STScI, which is operated by Associated Universities for
Research in Astronomy, Inc. (AURA), under NASA contract NAS 5-26555.
...
J.M.D acknowledges support of the consolider project CSD2010-00064 and
...
A.Z. acknowledges financial support from NASA through grant
